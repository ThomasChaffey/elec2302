...
(function () {
  ...
  //   GoatCounter event is sent, named  widget-week2-<name>  (e.g.
  //   widget-week2-sample). Nothing is drawn on the page and nothing is
  ...
  //   listed alongside pages. The "visits" number for widget-week2-sample is
  ...
  const TRACK = (function () {
    ...
    const WEEK = "week2";                      // namespaces events across weeks
  ...
  // ========================================================================
  // (L2) Sampling a signal: stems at spacing Δ; let Δ → 0 and the dot product
  //      Σ u_t y_t becomes the integral ⟨u,y⟩.
  // ========================================================================
  function sampling() {
    const host = document.getElementById("w-sample"); if (!host) return;
    const W = 720, H = 300, cv = document.createElement("canvas");
    host.appendChild(cv); const ctx = dpiCanvas(cv, W, H);
    const T0 = -1, T1 = 9, x0 = 40, span = W - 70, cy = H / 2, amp = 92;
    const xt = t => x0 + (t - T0) / (T1 - T0) * span;
    const sig = t => Math.cos(t) + 0.4 * Math.cos(2 * t + 1.0);
    let dt = 0.9;
    function draw() {
      ctx.clearRect(0, 0, W, H);
      axes(ctx, xt(0), cy, x0 - 6, x0 + span + 6, cy - amp - 24, cy + amp + 24, "t", "");
      // the underlying continuous signal
      ctx.strokeStyle = NAVY; ctx.lineWidth = 2; ctx.beginPath();
      for (let u = 0; u <= span; u++) {
        const t = T0 + u / span * (T1 - T0), px = x0 + u, py = cy - amp * sig(t) / 1.4;
        u ? ctx.lineTo(px, py) : ctx.moveTo(px, py);
      }
      ctx.stroke();
      // stems at the sample instants
      let n = 0;
      ctx.lineWidth = 1.4;
      for (let t = 0; t <= T1; t += dt) {
        const px = xt(t), py = cy - amp * sig(t) / 1.4;
        ctx.strokeStyle = OUT; ctx.beginPath(); ctx.moveTo(px, cy); ctx.lineTo(px, py); ctx.stroke();
        ctx.fillStyle = OUT; ctx.beginPath(); ctx.arc(px, py, 2.6, 0, 2 * Math.PI); ctx.fill();
        n++;
      }
      for (let t = -dt; t >= T0; t -= dt) {
        const px = xt(t), py = cy - amp * sig(t) / 1.4;
        ctx.strokeStyle = OUT; ctx.beginPath(); ctx.moveTo(px, cy); ctx.lineTo(px, py); ctx.stroke();
        ctx.fillStyle = OUT; ctx.beginPath(); ctx.arc(px, py, 2.6, 0, 2 * Math.PI); ctx.fill();
        n++;
      }
      ctx.fillStyle = NAVY; ctx.font = "14px Arial"; ctx.fillText("x(t)", xt(0) + 6, cy - amp - 14);
      ctx.fillStyle = INK; ctx.font = "13px Arial";
      ctx.fillText("sample spacing \u0394 = " + dt.toFixed(2) + "    samples shown: " + n,
        x0 + 6, cy + amp + 40);
      ctx.fillStyle = "#666"; ctx.font = "12px Arial";
      ctx.fillText("\u03A3 u\u209C y\u209C \u0394  \u2192  \u222B u(t) y(t) dt   as  \u0394 \u2192 0",
        x0 + 6, cy + amp + 58);
    }
    const bar = document.createElement("div");
    bar.style.cssText = "display:flex;align-items:center;gap:10px;margin-top:8px;font:14px Arial;color:" + INK;
    const lab = document.createElement("span"); lab.textContent = "sampling \u0394";
    const sl = document.createElement("input");
    sl.type = "range"; sl.min = 0.06; sl.max = 1.2; sl.step = 0.01; sl.value = dt;
    sl.style.cssText = "flex:1;accent-color:" + NAVY + ";";
    sl.addEventListener("input", () => { TRACK.hit(host.id, "slider"); dt = parseFloat(sl.value); draw(); });
    bar.appendChild(lab); bar.appendChild(sl); host.appendChild(bar);
    draw();
  }

  // ========================================================================
  // (L2) Delta sequence: one-sided triangle u_ε, base ε, height 2/ε, area 1.
  //      Drag ε → 0 and watch it narrow and grow.
  // ========================================================================
  function deltaSeq() {
    const host = document.getElementById("w-delta"); if (!host) return;
    const W = 560, H = 300, cv = document.createElement("canvas");
    host.appendChild(cv); const ctx = dpiCanvas(cv, W, H);
    const T0 = -0.6, T1 = 2.6, YMAX = 5;
    const x0 = 60, span = W - 100, ybase = H - 60, ytop = 24;
    const xt = t => x0 + (t - T0) / (T1 - T0) * span;
    const yv = v => ybase - v / YMAX * (ybase - ytop);
    let eps = 1.6;
    function draw() {
      ctx.clearRect(0, 0, W, H);
      const hgt = 2 / eps, clipped = hgt > YMAX;
      const yApex = clipped ? ytop : yv(hgt);
      // filled triangle: vertical leg at t=0, hypotenuse down to (ε,0)
      ctx.fillStyle = "rgba(0,70,140,0.10)";
      ctx.beginPath(); ctx.moveTo(xt(0), ybase); ctx.lineTo(xt(0), yApex);
      if (clipped) ctx.lineTo(xt(eps * (1 - YMAX / hgt)), ytop);
      ctx.lineTo(xt(eps), ybase); ctx.closePath(); ctx.fill();
      ctx.strokeStyle = NAVY; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.moveTo(xt(0), ybase); ctx.lineTo(xt(0), yApex);
      if (clipped) ctx.lineTo(xt(eps * (1 - YMAX / hgt)), ytop);
      ctx.lineTo(xt(eps), ybase); ctx.stroke();
      // axes last so the baseline reads on top of the fill
      axes(ctx, xt(0), ybase, x0 - 10, x0 + span + 6, ytop - 12, ybase + 20, "t", "");
      // height marker 2/ε
      if (!clipped) {
        ctx.strokeStyle = GREY; ctx.setLineDash([3, 3]); ctx.lineWidth = 1;
        ctx.beginPath(); ctx.moveTo(xt(0), yApex); ctx.lineTo(xt(eps) + 40, yApex); ctx.stroke();
        ctx.setLineDash([]);
        arrow(ctx, xt(eps) + 30, ybase, xt(eps) + 30, yApex, GREY, 1);
        ctx.fillStyle = INK; ctx.font = "13px Arial";
        ctx.fillText("2/\u03B5", xt(eps) + 36, (ybase + yApex) / 2);
      }
      // ε marker on the t axis
      ctx.fillStyle = INK; ctx.font = "13px Arial";
      ctx.fillText("0", xt(0) - 10, ybase + 16);
      ctx.fillText("\u03B5", xt(eps) - 3, ybase + 16);
      ctx.fillStyle = NAVY; ctx.font = "14px Arial";
      ctx.fillText("u\u03B5(t)", xt(0) - 44, ybase - 10);
      ctx.fillStyle = INK; ctx.font = "13px Arial";
      ctx.fillText("\u03B5 = " + eps.toFixed(2) + "    height 2/\u03B5 = " + hgt.toFixed(2) +
        (clipped ? "  (clipped)" : "") + "    area = \u00BD \u00B7 \u03B5 \u00B7 2/\u03B5 = 1", x0 - 10, H - 18);
    }
    const bar = document.createElement("div");
    bar.style.cssText = "display:flex;align-items:center;gap:10px;margin-top:8px;font:14px Arial;color:" + INK;
    const lab = document.createElement("span"); lab.textContent = "\u03B5";
    const sl = document.createElement("input");
    sl.type = "range"; sl.min = 0.05; sl.max = 2.0; sl.step = 0.01; sl.value = eps;
    sl.style.cssText = "flex:1;accent-color:" + NAVY + ";";
    sl.addEventListener("input", () => { TRACK.hit(host.id, "slider"); eps = parseFloat(sl.value); draw(); });
    bar.appendChild(lab); bar.appendChild(sl); host.appendChild(bar);
    draw();
  }

  function init() { euler(); sum(); ezt(); rlc(); sampling(); deltaSeq(); }
