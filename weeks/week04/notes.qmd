...
[tchaffey.com/elec2302](https://tchaffey.com/elec2302/?ref=notes-pdf-week4).
:::
...
In other words, $e^{\,j\omega t}$ is an eigenvector of the system. If we can express any $u$ in this basis, as a sum of sinusoids, this will "diagonalise the system" and "simplify analysis".
...
$u(t)$ arbitrary periodic signal on $[0, T]$.

$\omega_0 = \frac{2\pi}{T}$ is called the <u>fundamental frequency</u> of $u(t)$.
...
This is the <u>Fourier series</u> of $u$. It also turns out that each $c_k$ is independent, as $\phi_k$ are orthogonal, so the finite sum $\displaystyle\sum_{k=-n}^{n} \langle u,\phi_k\rangle_P\,\phi_k$ is a least-squares approximation of $u$ by $\{\phi_k\}_{k=-n,\dots,n}$. See the starred section of the notes for the full details.
...
### Starred section: why are the $c_k$ independent?
...
In this case, we can also derive a sin/cosine form of the Fourier series.
...
The signal $u$ is real and even, so $c_k = \overline{c_{-k}}$ and $c_k = c_{-k}$, which forces every $c_k$ to be real. This is what the calculation gives.
...
The DC term $2/\pi$ is the average value of the rectified sine, and the harmonics fall off like $1/k^2$.
