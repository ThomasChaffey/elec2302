...
This is where every quiz happens.
...
## The quizzes are hurdles {.smaller}
...
attempt is normal — it is how these are designed to work, not a sign that you are behind.
:::

- An HD in Quiz 2 or Quiz 3 unlocks a Bonus Quiz, where you can earn up to 5% top up marks.

## When your quizzes happen {.smaller}

Quizzes run in tutorials, so your attempts land in the weeks you have a tutorial rather
than a lab. One rule covers all of it: [your attempts are in your next tutorials.]{.navy}
...
timetable. Your weeks are on your timetable — check them now.
:::
...
## What a quiz looks like {.smaller}
...
You get a [formula sheet]{.navy} and can use a calculator. The formula sheet is on Canvas.
...
A **signal** carries information: a voltage on a wire, the price of a barrel of oil, the
...
Knight Capital, 1 August 2012. A trading system went live, ran for 45 minutes, and lost
...
accelerating hard, so the position is measured continuously and fed back to the motors.
...
## Example 6: the FFT in your pocket {.smaller}

::: {.shot}
![](FFT.png)
:::

Wi-Fi and 5G do not use the Fourier transform because it is convenient. The standards
[define]{.navy} the transmitted signal as a sum of complex exponentials — an inverse
Fourier transform.

So every phone ships an FFT engine in silicon. In Wi-Fi it runs a 64-point transform every
4 µs: [a quarter of a million transforms a second]{.navy}, whenever the radio is on.

::: {.credit}
LTE-FFT IP core, AMD. Waveform defined in IEEE Std 802.11a-1999 §17.3.5.9 and 3GPP TS 36.211 §6.12.
:::

## Example 7: convolution in silicon {.smaller}

::: {.shot}
![](convolution.png)
:::

A neural network built from memristor crossbars. The convolution is not computed by a
processor: it happens in the array itself, by [Ohm's law and Kirchhoff's current law]{.navy}.
...
Today's lecture: the exponential function, Euler's formula,
