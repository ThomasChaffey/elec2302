...
algebra**, **complex numbers** and **trigonometry**. This lecture revises the core of
that intersection — the material everything else is built on.
...
Turns out $f(z) = e^{z}$ is the unique solution to this differential equation (it is an
...
Why: let's now represent sinusoids as complex numbers. Let
...
Why use complex numbers to represent sinusoids? It makes the algebra easier. For example,
...
The triangle $3 + 4j = 5\angle 53.1^{\circ}$ simply rotates — that is the sum.
...
Drag $z$ around the plane and watch the trajectory of $e^{zt}$ and its real part respond:
...
![Series RLC circuit (drawn with circuitikz).](circuit-rlc.svg){width=540}
:::
...
&= R I e^{j\omega t} + L\frac{\mathrm{d}}{\mathrm{d}t}I e^{j\omega t}
   + \frac{1}{C}\!\int_{-\infty}^{t}\! I e^{j\omega\tau}\,\mathrm{d}\tau \\[2pt]
&= R I e^{j\omega t} + j\omega L\, I e^{j\omega t} + \frac{I}{j\omega C}\,e^{j\omega t}.
\end{aligned}
$$
...
Drag the reactance to watch the input and output phasors and waveforms:
