...
[Other formats:]{.label}
```{=html}
<a href="notes.pdf" title="Week 1 lecture notes as a PDF">&#128196;&nbsp;PDF</a>
```
:::
:::
...
::: {.content-visible when-format="pdf"}
![The unit vector $e^{j\theta}$ and its imaginary part unrolling into a
sinusoid.](figs/w-euler.png){#fig-euler width=100%}
:::
...
::: {.content-visible when-format="pdf"}
![Adding $3$ and $4j$ tip to tail. The resultant $5\angle 0.927$ rotates, and its
real part traces $5\cos(\omega t + 0.927)$.](figs/w-sum.png){#fig-sum width=52%}
:::
...
::: {.content-visible when-format="pdf"}
![Left: the position of $z$. Middle: the trajectory of $e^{zt}$ in $\mathbb{C}$.
Right: its real part against $t$. Shown for $z = j$, giving pure
oscillation.](figs/w-ezt.png){#fig-ezt width=100%}
:::
...
::: {.content-visible when-format="pdf"}
![Input and output phasors (left) and waveforms (right), at $R = 30\,\Omega$ and
$\omega L - 1/\omega C = 40\,\Omega$, so $|Z| = 50\,\Omega$ and $\angle Z = 0.927$.
Amplitudes are normalised here; the live version can show the output at its true
size, $R/|Z| = 0.60$ of the input.](figs/w-rlc.png){#fig-rlc width=100%}
:::
